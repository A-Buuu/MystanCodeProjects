...
def get_best_pixel(pixels):
    ...
    pixel_dist_dict = {}
    for pixel in pixels:
        pixel_dist_dict[pixel] = get_pixel_dist(pixel, avg_pixel[0], avg_pixel[1], avg_pixel[2])
    sorted_list = sorted(pixel_dist_dict.items(), key=lambda ele: ele[1])
    best = sorted_list[0][0]
...
def solve(images):
    ...
    for x in range(width):
        for y in range(height):
            img_pixels = []
            for img in images:
                img_pixels.append(img.get_pixel(x, y))
            best_pixel = get_best_pixel(img_pixels)
